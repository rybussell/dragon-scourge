...
function updateuserrow() {
    ...
    $query = doquery("UPDATE ".$prefix."_users SET $querystring WHERE id='".$userrow["id"]."' LIMIT 1");
    
}

function builditem($wpre, $baseitem, $suffix, $modrow) { // Copy of town.php's builditem().
    ...
    if ($wpre != false) {
        $baseitem["fullid"] = $wpre["id"] . "," . $baseitem["fullid"];
        $baseitem["name"] = $wpre["name"] . " " . $baseitem["name"];
        $baseitem["buycost"] += $wpre["buycost"];
        $baseitem["sellcost"] += $wpre["sellcost"];
        $baseitem["reqlevel"] = max($baseitem["reqlevel"], $wpre["reqlevel"]);
        $baseitem["reqstrength"] += $wpre["reqstrength"];
        $baseitem["reqenergy"] += $wpre["reqenergy"];
        $baseitem["reqdexterity"] += $wpre["reqdexterity"];
        $baseitem["itemmods"] .= $modrow[$wpre["basename"]]["prettyname"] . ": +" . $wpre["baseattr"];
        if ($modrow[$wpre["basename"]]["percent"] == 1) { $baseitem["itemmods"] .= "%"; }
        $baseitem["itemmods"] .= "<br />\n";
