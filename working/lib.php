...
function uber_mres($value) {
    $value = is_array($value) ? array_map('uber_mres', $value) : mysql_real_escape_string($value);
    return $value;
}
...
function display($title, $content, $panels = true) { // Finalize page and output to browser.
    ...
    if (!isset($controlrow)) {
        $controlrow = dorow(doquery("SELECT * FROM ".$prefix."_control WHERE id='1' LIMIT 1"));
    }
...
";
    ...
    if ($controlrow["debug"] == 1) {
        $row["debug"] = "/ " . $numqueries . " Queries / " . round(getmicrotime() - $starttime, 4) . " Seconds";
    } else {
        $row["debug"] = "";
    }
    if ($row["moddedby"] != "") {
