...
function uber_ss($value) {
    $value = is_array($value) ? array_map('uber_ss', $value) : stripslashes($value);
    return $value;
}

function uber_mres($value) {
    $value = is_array($value) ? array_map('uber_mres', $value) : mysqli_real_escape_string($value);
    return $value;
}
function uber_hsc($value) {
    $value = is_array($value) ? array_map('uber_hsc', $value) : htmlspecialchars($value);
    return $value;
}

function doquery($link,$query) { // Something of a tiny little database abstraction layer.
    include ('config.php');
    global $numqueries, $controlrow;
    $sqlquery = mysqli_query($link,$query);
    if ($sqlquery == false) {
        if ($controlrow["debug"] == 1) {
            die(mysqli_error() . "<br /><br />" . $query);
        } else {
...
function dorow($link,$sqlquery, $force = "") { // Abstraction layer part deux.
...
function display($title, $content, $panels = true) { // Finalize page and output to browser.
    ...
    if (!isset($controlrow)) {
        $controlrow = dorow(doquery($link,"SELECT * FROM ".$prefix."_control WHERE id='1' LIMIT 1"));
    }
...
if ($controlrow["forumurl"] != "") {
        $row["forumslink"] = "<a href=\"" . $controlrow["forumurl"] . "\">Support Forums</a>";
    } else {
        $row["forumslink"] = "";
    }
    if ($controlrow["debug"] == 1) {
        $row["debug"] = "/ " . $numqueries . " Queries / " . round(microtime() - $starttime, 4) . " Seconds";
    } else {
        $row["debug"] = "";
    }
    if ($row["moddedby"] != "") {
